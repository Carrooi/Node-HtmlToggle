(function() {
	...
	(function() {
	 ...
	 '/tests/Toggle.coffee': function(exports, module) {
	
		/** node globals **/
		var require = function(name) {return __r__c__.require(name, '/tests/Toggle.coffee');};
		require.resolve = function(name, parent) {if (parent === null) {parent = '/tests/Toggle.coffee';} return __r__c__.require.resolve(name, parent);};
		require.define = function(bundle) {__r__c__.require.define(bundle);};
		require.cache = __r__c__.require.cache;
		var __filename = '/tests/Toggle.coffee';
		var __dirname = '/tests';
		var process = {cwd: function() {return '/';}, argv: ['node', '/tests/Toggle.coffee'], env: {}};
	
		/** code **/
		(function() {
		  describe('Toggle', function() {
		    it('should throw an error if attribute data-target does not exists', function() {
		      return expect(function() {
		        return Toggle($, '#testOne');
		      }).to["throw"](Error, 'Toggle: missing data-target attribute.');
		    });
		    it('should throw an error if element from target attribute does not exists', function() {
		      return expect(function() {
		        return Toggle($, '#testTwo');
		      }).to["throw"](Error, "Toggle: element '#testTwo .unknown' was not found.");
		    });
		    it('should toggle target element', function() {
		      Toggle($, '#testThree');
		      $('#testThree a').click();
		      expect($('#testThree div').is(':hidden')).to.be["true"];
		      $('#testThree a').click();
		      return expect($('#testThree div').is(':visible')).to.be["true"];
		    });
		    return it('should toggle target element and hide toggler', function() {
		      Toggle($, '#testFour');
		      expect($('#testFour a').is(':visible')).to.be["true"];
		      $('#testFour a').click();
		      return expect($('#testFour a').is(':hidden')).to.be["true"];
		    });
		  });
	...
	__r__c__.require.__setStats({"/tests/Toggle.coffee":{"atime":1391100992000,"mtime":1391100988000,"ctime":1391100988000}});
